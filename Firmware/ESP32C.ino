...
const char* PI_IP = "192.168.1.150";
// --------------------------------------------------------------------------
...
constexpr uint8_t LED_AMBER = 17;
...
void testIndicators() {
  ...
  delay(250);
  writeLed(LED_GREEN, false);
  writeLed(LED_AMBER, true);
  delay(250);
  writeLed(LED_AMBER, false);
  writeLed(LED_RED, true);
  delay(250);
  allLedsOff();
  Serial.println("[TEST] LEDs: GREEN -> AMBER -> RED");
}
...
void setIndicators() {
  ...
  if (alignmentState == "DEGRADED") {
    ...
    // WAITING/acquiring: amber is a useful visible diagnostic.
    writeLed(LED_AMBER, true);
  }
}
